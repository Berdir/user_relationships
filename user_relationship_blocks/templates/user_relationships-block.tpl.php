...
if ($relationships) {
  ...
  foreach ($relationships as $rtid => $relationship) {
    $tt_rel_name = user_relationships_type_get_name($relationship);
    $tt_rel_plural_name = user_relationships_type_get_name($relationship, TRUE);
    if ($the_other_uid == $relationship->requester_id) {
      $rtype_heading = $relationship->is_oneway ? 
        ...
        t("@rel_plural_name", user_relationships_type_translations($relationship));
